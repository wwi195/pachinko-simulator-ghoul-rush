...
const { LUCKY_REACH_DIGIT, LUCKY_REACH_DIGIT_RATE, rollReachDigit } = require('../rush-view-engine.js');

test('LUCKY_REACH_DIGIT は7、LUCKY_REACH_DIGIT_RATE は5%(お楽しみ用の初期値、後で調整可能)', () => {
  assert.equal(LUCKY_REACH_DIGIT, 7);
  assert.equal(LUCKY_REACH_DIGIT_RATE, 0.05);
});

test('rollReachDigit: rng<LUCKY_REACH_DIGIT_RATEなら必ずLUCKY_REACH_DIGIT(7)になる', () => {
  const d = withMockRandom([0], () => rollReachDigit());
  assert.equal(d, LUCKY_REACH_DIGIT);
});

test('rollReachDigit: rng>=LUCKY_REACH_DIGIT_RATEなら7を除く1〜8から選ばれる(2回目のrngで決定、先頭は1)', () => {
  const d = withMockRandom([0.99, 0], () => rollReachDigit());
  assert.equal(d, 1);
});

test('rollReachDigit: 2回目のrngが最大に近い場合は8になる(7を除いた末尾の値)', () => {
  const d = withMockRandom([0.99, 0.999], () => rollReachDigit());
  assert.equal(d, 8);
});

const { forceRushHit } = require('../rush-view-engine.js');

test('forceRushHit: 必ずhit_small/hit_bigになりmiss/st_endにはならず、STが130回転にリセットされ連チャン数が+1される', () => {
  const before = { stRemaining: 1, chainCount: 2, bonus3000: 1, bonus6000: 0, actualBalls: 1000, nominalBalls: 1000 };
  // rollRushHitType()の抽選:0.999 >= P_RUSH_BIG(0.03) → small
  const result = withMockRandom([0.999], () => forceRushHit(before));
  assert.equal(result.outcome, 'hit_small');
  assert.equal(result.rushState.stRemaining, 130); // RUSH_ST_COUNT
  assert.equal(result.rushState.chainCount, 3);
});

test('forceRushHit: rollRushHitTypeの抽選次第でhit_bigにもなる', () => {
  const before = { stRemaining: 50, chainCount: 0, bonus3000: 0, bonus6000: 0, actualBalls: 0, nominalBalls: 0 };
  // rollRushHitType()の抽選:0 < P_RUSH_BIG(0.03) → big
  const result = withMockRandom([0], () => forceRushHit(before));
  assert.equal(result.outcome, 'hit_big');
});
